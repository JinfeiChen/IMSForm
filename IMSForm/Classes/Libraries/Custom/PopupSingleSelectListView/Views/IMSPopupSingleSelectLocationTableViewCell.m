//
//  IMSPopupSingleSelectLocationTableViewCell.m
//  IMSForm
//
//  Created by cjf on 26/2/2021.
//

#import "IMSPopupSingleSelectLocationTableViewCell.h"
#import <Masonry/Masonry.h>
#import <IMSForm/IMSFormValidateManager.h>

@interface IMSPopupSingleSelectLocationTableViewCell ()
...
@implementation IMSPopupSingleSelectLocationTableViewCell
...
- (void)setModel:(IMSPopupSingleSelectLocationModel *)model {
    _model = model;

    NSString *name = (model.Name ? : model.label);
    self.nameLabel.text = [IMSFormValidateManager isEmpty:name] ? @"N/A" : name;
    if (![IMSFormValidateManager isEmpty:model.CountryCode]) {
        self.roleLabel.text = [NSString stringWithFormat:@" %@ ", (model.CountryCode ? : @"N/A")];
    }
    NSString *phone = (model.Phone && (model.Phone.length > 0)) ? model.Phone : @"";
    NSString *url = (model.URL && (model.URL.length > 0)) ? model.URL : @"";
    NSString *seperate = (phone.length && url.length) ? @" / " : @"";
    self.phoneLabel.text = [NSString stringWithFormat:@"%@%@%@", phone, seperate, url];
    self.infoLabel.text = [NSString stringWithFormat:@"%@", model.FormattedAddressLines ? model.FormattedAddressLines.firstObject : @"N/A"];
