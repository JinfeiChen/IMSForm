...
#import "IMSValuePopupView.h"
#import <Masonry/Masonry.h>

#define ARROW_LENGTH 3
#define MIN_POPUPVIEW_WIDTH 30
#define MIN_POPUPVIEW_HEIGHT 10.0
#define POPUPVIEW_WIDTH_INSET 3

#define POP_TRANSFORM @"transform"
#define POP_OPACITY   @"opacity"

NSString *const AnimationLayer = @"animationLayer";
NSString *const FillColorAnimation = @"fillColor";

@interface IMSValuePopupView ()<CAAnimationDelegate>

@end

@implementation IMSValuePopupView
{
...
- (void)setTextColor:(UIColor *)textColor {
    ...
    [_attributedString addAttribute:NSForegroundColorAttributeName value:(id)textColor.CGColor range:NSMakeRange(0, [_attributedString length])];
}
...
- (void)show {
    ...
    [CATransaction begin]; {
        ...
        NSValue *fromValue = [self.layer animationForKey:POP_TRANSFORM] ? [self.layer.presentationLayer valueForKey:POP_TRANSFORM] : [NSValue valueWithCATransform3D:CATransform3DMakeScale(0.5, 0.5, 1)];
        
        CABasicAnimation *scaleAnim = [CABasicAnimation animationWithKeyPath:POP_TRANSFORM];
        ...
        [self.layer addAnimation:scaleAnim forKey:POP_TRANSFORM];
        
        CABasicAnimation *fadeInAnim = [CABasicAnimation animationWithKeyPath:POP_OPACITY];
        fadeInAnim.fromValue = [self.layer.presentationLayer valueForKey:POP_OPACITY];
        fadeInAnim.duration = 0.1;
        fadeInAnim.toValue = @1.0;
        [self.layer addAnimation:fadeInAnim forKey:POP_OPACITY];
...
- (void)hide {
    ...
    [CATransaction begin]; {
        CABasicAnimation *scaleAnim = [CABasicAnimation animationWithKeyPath:POP_TRANSFORM];
        scaleAnim.fromValue = [self.layer.presentationLayer valueForKey:POP_TRANSFORM];
        ...
        CABasicAnimation *fadeOutAnim = [CABasicAnimation animationWithKeyPath:POP_OPACITY];
        fadeOutAnim.fromValue = [self.layer.presentationLayer valueForKey:POP_OPACITY];
        fadeOutAnim.toValue = @0.0;
        fadeOutAnim.duration = 0.8;
        [self.layer addAnimation:fadeOutAnim forKey:POP_OPACITY];
...
- (void)layoutSubviews {
    ...
    if (!CGSizeEqualToSize(self.bounds.size, _oldSize)) {
        ...
        NSAttributedString *mutString = _textLayer.string;
        CGFloat textHeight = [mutString size].height;
        CGRect textRect = CGRectMake(self.bounds.origin.x, (self.bounds.size.height-ARROW_LENGTH-textHeight)/2, self.bounds.size.width, textHeight);
